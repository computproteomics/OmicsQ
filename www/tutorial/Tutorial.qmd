...
*Online documentation*: Further information about the different operations of the user interface is available through the information ({{< fa info-circle >}}) buttons.
...
[Step IV: Analysis via External Apps](#step-iv-analysis-via-external-apps)
...
- Retrieve and incorporate results back into OmicsQ to have a single file with all information.
...
The OmicsQ manuscript: [preprint](https://arxiv.org/abs/2504.19813)

For the publications of the other apps, see the section [About the external Apps](#details-about-the-external-apps)
...
Download the example file containing protein abundances of a cell differentiation experiment from the GitHub repository [here](https://github.com/computproteomics/OmicsQ/blob/main/data/Myo.csv). You can use the *download raw file* button for direct download. Then upload the file into OmicsQ.

_Description of data set_: 
Immortalized human satellite cells (KM155C25) were differentiated to myocytes and sampled in biological triplicates at Day −1 (activated myoblasts) and Days 0–4; each sample was measured in technical triplicates. Proteins were prepared using published protocols and analyzed by 120-min nanoLC–MS/MS on an Orbitrap Fusion Lumos in DDA/HCD mode. Raw data were processed in MaxQuant (v1.5.5.1) with Andromeda against the human UniProt Reference Proteome. The dataset is available via ProteomeXchange (PXD018588). A more detailed description is available in the [PolySTest paper](https://pubmed.ncbi.nlm.nih.gov/32424025/).
...
The `id` column has already been chosen correctly as indicated by a "Type" `id` above the column. 

Now press the button below *Select quantitative columns*. You can write "LFQ" in the text field and then use the *select all* option to get all columns but the one for the identifications. Then click anywhere in the app. Now all quantitative columns should be marked as `quant`.
...
Play with the rules of the string distance to see how the sample grouping changes. A high Levenstein distance threshold should
give you the correct assignment.
...
Change the slider to separate the experimental groups (setting it to one should do it).
...
### 1. Add or Delete Data Columns & Summary
...
### 3. UniProt Mapping & Proceed

- *UniProt Mapping*: Standardize protein identifiers by mapping your data to UniProt accessions (for usage in ComplexBrowser). You can select your identifier type (Ensembl, Gene Names, 
etc.) or let the app auto-detect it. Skip this if your data already uses UniProt 
IDs. <u>Note: the mapping is done via UniProt API and thus will most likely take a while.</u>
...
- *PCA Plot*: Shows the separation of samples by the first two principal components. Use the 'How to show labels' option to toggle sample names on/off. 
...
*Missing Values Plot*: View distribution over features or missing values per sample.
...
## Step IV: Analysis via External Apps
